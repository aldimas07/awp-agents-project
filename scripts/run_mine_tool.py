...
# when it's called from scripts/. The actual mine_skill source is in src/python/mine_skill.
PROJECT_ROOT = SCRIPT_DIR.parent # This is awp-agents-project/
...
def _ensure_local_venv_python() -> None:
    if os.environ.get("MINE_SKIP_VENV_REEXEC") == "1":
        return
    # Resolve venv python relative to PROJECT_ROOT
    ...
    # Use SCRIPT_DIR to refer to the actual run_mine_tool.py script location
    argv = [str(target), str(SCRIPT_DIR / "run_mine_tool.py"), *sys.argv[1:]]
    if os.name == "nt":
...
try:
    ...
    print(
        ...
        f"Run setup script:  bash {PROJECT_ROOT}/scripts/setup.sh\n"
        f"Or install manually:  pip install -r {PROJECT_ROOT}/src/python/requirements.txt",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def _bootstrap_command() -> str:
    if os.name == "nt":
        # Assuming setup.ps1 will be in scripts/
        return f"powershell -ExecutionPolicy Bypass -File {PROJECT_ROOT / 'scripts' / 'setup.ps1'}"
    # Assuming setup.sh will be in scripts/
    return str(PROJECT_ROOT / "scripts" / "setup.sh")
...
def _default_output_root() -> Path:
    # output/agent-runs is now under agents/AGENT_ID/logs for predict, and we'll use that for mine logs too
    # WorkerStateStore uses WORKER_STATE_ROOT env var, which is set per agent in hive.sh
    # If CRAWLER_OUTPUT_ROOT is not set, use a global data dir.
    return Path(os.environ.get("CRAWLER_OUTPUT_ROOT", str(PROJECT_ROOT / "data" / "_crawler_output"))).resolve()


def _default_state_root() -> Path:
    # worker_state is now under agents/AGENT_ID/state, set by WORKER_STATE_ROOT env var.
    # If WORKER_STATE_ROOT is not set, use a global data dir.
    return Path(os.environ.get("WORKER_STATE_ROOT", str(PROJECT_ROOT / "data" / "_worker_state"))).resolve()


def _default_browser_auth_root() -> Path:
    return (PROJECT_ROOT / "data" / "_browser_auth").resolve() # Move to global data dir
...
def _spawn_browser_session_waiter(platform: str) -> int:
    ...
    with log_path.open("a", encoding="utf-8") as handle:
        ...
        process = subprocess.Popen(
            [sys.executable, str(SCRIPT_DIR / "run_mine_tool.py"), "browser-session-wait", platform],
